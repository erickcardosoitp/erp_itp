...
6. **Soft delete (v22) não cobre SQL cru** — `@DeleteDateColumn` filtra automaticamente `find()`/`findOne()` do TypeORM, mas as dezenas de queries SQL manuais em `relatorios.service.ts`/`publico.service.ts`/`academico.service.ts` contra `movimentacoes_financeiras` **não** têm o filtro `deleted_at IS NULL`. Retrofit correto exige revisar cada query individualmente — não foi feito ainda, ver commit `feat(db): soft delete...` pra escopo exato do que foi coberto.
