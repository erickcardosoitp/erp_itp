...
export class AppService {
  ...
  async monitoramentoTI() {
    ...
    let max_conexoes = 0;
    let tabelas: { nome: string; tamanho: string; tamanho_bytes: number; linhas: number }[] = [];
    ...
    // Tamanho do banco — funciona no Neon
    try {
      ...
      db_tamanho       = row?.tamanho ?? '–';
      db_tamanho_bytes = parseInt(row?.tamanho_bytes ?? '0');
    } catch { /* ignorado */ }

    // Cache hit ratio — pg_stat_database (pode não estar disponível no Neon)
    try {
      const [row] = await this.dataSource.query(
        `SELECT round(
      ...
      cache_hit_pct = row?.hit_pct != null ? parseFloat(row.hit_pct) : null;
    } catch { /* ignorado */ }

    // Conexões — pg_stat_activity (pode ser restrito no Neon)
    try {
      const [row] = await this.dataSource.query(
        `SELECT
      ...
      if (row) {
        conexoes_ativas = parseInt(row.ativas   ?? '0');
        conexoes_totais = parseInt(row.totais   ?? '0');
        max_conexoes    = parseInt(row.max_conn ?? '0');
      }
    } catch { /* ignorado */ }

    // Tabelas por tamanho — pg_tables + pg_class (geralmente disponível)
    try {
      const rows = await this.dataSource.query(
        `SELECT
           ...
           coalesce(c.reltuples::bigint, 0) AS linhas
         FROM pg_tables t
      ...
      tabelas = rows.map((r: any) => ({
        nome:          r.nome,
        tamanho:       r.tamanho,
        tamanho_bytes: parseInt(r.tamanho_bytes ?? '0'),
