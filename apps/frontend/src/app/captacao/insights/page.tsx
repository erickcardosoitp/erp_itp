...
import type { PipelineStatus, SourceType } from '../types';
    ...
    <div className="space-y-6">
      ...
      {kpis.avg_score > 0 && (
        <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-2xl p-4 flex items-center justify-between">
          <div>
            <p className="text-xs font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider">Score médio IA</p>
            <p className="text-3xl font-black text-purple-700 dark:text-purple-300">{kpis.avg_score.toFixed(0)}</p>
          </div>
          <div className="text-right space-y-1">
            <p className="text-xs text-slate-500">Aprovado: {formatBRL(kpis.value_approved)}</p>
            <p className="text-xs text-slate-500">Submetido: {formatBRL(kpis.value_submitted)}</p>
          </div>
        </div>
      )}
    </div>
