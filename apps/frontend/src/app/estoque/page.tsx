...
export default function EstoquePage() {
  ...
  const [formProduto, setFormProduto] = useState<any>({ categoria: 'Geral', unidade_medida: 'un', estoque_minimo: 0, quantidade_atual: 0 });
  ...
  // ── Produto CRUD ──────────────────────────────────────────────────────────
  const abrirNovo = () => { setFormProduto({ categoria: 'Geral', unidade_medida: 'un', estoque_minimo: 0, quantidade_atual: 0 }); setErro(''); setModalProduto({ aberto: true, editando: null }); };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <form onSubmit={salvarProduto} className="px-6 py-5 space-y-4">
              ...
              <FInput label="Categoria" value={formProduto.categoria ?? 'Geral'} onChange={v => setFormProduto((p: any) => ({ ...p, categoria: v }))} placeholder="Ex: Alimentos, Limpeza, Material" />
